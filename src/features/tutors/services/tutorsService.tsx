...
export const exportFullTutors = async (format: 'json' | 'sql' | 'csv' | 'xlsx' = 'json'): Promise<void> => {
  try {
    ...
    if (format === 'json') {
      ...
      const a = document.createElement('a'); a.href = url; a.download = `tutores-${stamp}.json`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    } else {
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href = url; a.download = `tutores-${stamp}.${ext}`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    }
  } catch (error) {
    console.error('[tutorsService] Error al exportar tutores:', error);
    throw error;
  }
};
